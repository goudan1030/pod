# Pacdora 3D模型渲染流程深度分析

## 📋 概述

本文档深入分析 Pacdora 网站从 3D 模型加载、拆解、用户上传贴图到最终渲染的完整技术流程。
...
### 2. 模型文件格式
- **自定义格式**: `.pda` 文件 (Pacdora Design Archive)
- **存储位置**: `cdn.pacdora.com/static/blender/{uuid}.pda`
- **来源**: 从 Blender 导出，包含几何体、UV、材质信息

### 3. 纹理系统
- **格式**: JPG/PNG
- **存储**: `cdn.pacdora.com/texture/{uuid}.jpg`
- **支持**: 多纹理层级、用户自定义贴图

### 4. 环境光照
- **HDR 贴图**: `cdn.pacdora.com/hdr/{uuid}.hdr`
- **用途**: 环境反射、真实光照效果
...
```javascript
{
  ...
  blenderFileUrl: "cdn.pacdora.com/static/blender/{uuid}.pda",
  parts: [
    {
      partId: "front",
      partName: "正面",
  ...
  defaultMaterials: [...],
  hdrEnvironment: "cdn.pacdora.com/hdr/{uuid}.hdr"
}
```
...
// 返回: "cdn.pacdora.com/texture/{uuid}.jpg"
```
...
const userTexture = textureLoader.load(
  'cdn.pacdora.com/texture/{uuid}.jpg',
  (texture) => {
    // 纹理加载完成回调
...
hdrLoader.load('cdn.pacdora.com/hdr/{uuid}.hdr', (texture) => {
  ...
  "materials": [
    {
      ...
      "properties": {
        "roughness": 0.7,
        "metalness": 0.0,
        "normalMap": "cdn.pacdora.com/texture/cotton_normal.jpg",
        "aoMap": "cdn.pacdora.com/texture/cotton_ao.jpg"
      }
    },
    {
      "id": "polyester_black",
